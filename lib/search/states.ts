...
export const STATES: StateEntry[] = [
  ...
  { code: "DC", name: "Washington D.C.", slug: "washington-dc" },
];
...
export function stateByCode(code: string): StateEntry | undefined {
  return STATES.find((s) => s.code === code.toUpperCase());
}

export function matchingStates(query: string): StateEntry[] {
